...
class Parser:
    ...
    # Parse the measures out of the eedomus gateway
    ...
    # Parse the measures for all the sensors of one given room
    ...
    def parse_qai(self):
        ...
        raw = pd.read_pickle('../AirQuality/QAI_xxx')
        ...
        try :
            ...
            self.err.log('Could not parse the xxx database')

            return [], []

    # Parse the agenda for room schedule
...
class Database:
    ...
    # Defines a list of the available dates in the database given two entries
    ...
    # Add a new date and its corresponding measures
    def add_date_to_database(self, dte):
        ...
        if len(qai) == 0 : qai = list(np.empty(len(pd.read_pickle('../AirQuality/QAI_xxx').columns)) * np.NaN)
